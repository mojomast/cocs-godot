...
func run() -> void:
	...
	for cycle in range(3):
		for id: String in MapBuilder.IDS:
			...
			check(map.materials.size() == 5, "material reuse")
		...
		counts.append({"nodes":Performance.get_monitor(Performance.OBJECT_NODE_COUNT),"resources":Performance.get_monitor(Performance.OBJECT_RESOURCE_COUNT),"orphans":Performance.get_monitor(Performance.OBJECT_ORPHAN_NODE_COUNT)})
	...
	var result := {"assertions":assertions,"failures":failures,"cycles":counts,"scope":"Headless map build/free lifecycle, not gameplay restart acceptance"}
