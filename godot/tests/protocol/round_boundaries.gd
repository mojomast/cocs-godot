...
func _initialize() -> void:
	...
	for cycle: int in range(3):
		...
		var pickup_mesh_ref : WeakRef = weakref(session.pickups.markers[7].get_node("Housing"))
		...
		check(actor_ref.get_ref() == null and pickup_ref.get_ref() == null and mesh_ref.get_ref() == null and pickup_mesh_ref.get_ref() == null, "boundary frees visuals and descendants synchronously")
