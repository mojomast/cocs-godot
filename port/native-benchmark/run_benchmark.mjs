...
function parse(argv) {
  const options = {map: 'prism-foundry', bots: 4, resolution: '1280x800', level: 'all',
    roundSeconds: 180, seconds: 150, out: 'port/native-benchmark/evidence', label: '', keepTemp: false};
...
async function runOnce({options, binary, level, stamp, outDir}) {
  const runDir = join(outDir, `${stamp}-${options.resolution}-${level}`);
  ...
  const godotArgs = [binary, '--path', 'godot', '--audio-driver', 'Dummy', '--resolution', options.resolution,
    ...
    `--round-seconds=${options.roundSeconds}`, `--endpoint=${endpoint}`, '--benchmark'];
