...
for(const file of ['godot/world/session.gd','godot/world/projectiles.gd','godot/world/combat_feedback.gd','godot/world/audio_feedback.gd','godot/ui/match_setup.gd','port/native-projectile-combat/live.gd','port/native-projectile-combat/run.mjs'])summary.sourceHashes[file]=createHash('sha256').update(readFileSync(resolve(root,file))).digest('hex');
...
try{
  ...
  for(const [map,mode] of [...['meridian-exchange','verdant-reliquary','ember-crucible'].map(m=>[m,'rockets']),['meridian-exchange','deathmatch']]){
    ...
    const inputs={count:0,fire:0,movement:0}; const sourceEvents={launches:0,pickups:0};
    ...
    const onConnection=socket=>{
      ...
      socket.send=function(data,...args){
        ...
        if(f.type==='events')for(const e of f.items){if(e.actor===actorId&&e.type==='launch')sourceEvents.launches++;if(e.actor===actorId&&e.type==='pickup'&&e.kind==='rocket')sourceEvents.pickups++;}
        if(mode==='deathmatch'&&f.type==='snapshot'&&!planned){const a=f.state.actors.find(a=>a.id===actorId);if(a){writeFileSync(routePath,JSON.stringify(planRoute(DESTINATION_COMBAT_MAPS[0],a)));planned=true;}}
    ...
    if(mode==='deathmatch')assert.equal(sourceEvents.pickups,1);
