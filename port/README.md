...
This is an interactive diagnostic prototype, NOT the first-playable acceptance milestone. Actors are capsules and the level is semantic geometry. Pickup markers now follow authoritative snapshot positions and availability; they remain diagnostic cubes rather than original models. There is no weapon model, shot/hit presentation, dynamic obstacle presentation, audio or prediction. Remote actors now use receive-clock interpolation with a provisional 100ms delay; the local camera still follows authoritative snapshot cadence and local look responds immediately. The renderer has not been visually accepted. Race/soccer/LATTICE sessions are not exposed by this prototype; all nine maps remain in the separate viewer.
