...
| Nacre Engine Horde development build | Source team-0/1 spawn pools, twelve supported supply sites, five wave-gated weapon caches; source-validated local operator/harness selection; identity-map graybox, ray parity and full 185-gate serial verifier pass at `088dae66` | Two-size rendered startup and natural three-wave gameplay/results/restart captures in `port/native-identity-horde/evidence/2026-09-25T0*`; earlier capture exceeded the recorder's former 96 MiB bound, now 192 MiB | Accelerated ten-wave source transitions check each cache; natural three-wave run won with 12 kills, 3 lives, score338 and restart. Fresh Linux archive extraction passes 17 existing package cases plus packaged Nacre/Claude live snapshots; Windows archive 141/141 files and routing verified on Linux | [Nacre Horde prerelease](https://github.com/mojomast/cocs-godot/releases/tag/nacre-horde-2026-09-25) ships both targets; Windows native/hardware run, natural ten-wave champion/defeat and hands-on balance remain OPEN |
