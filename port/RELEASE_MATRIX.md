...
| Multiplayer lobby / spectator | Lobby33, strict notice-context66, lead actual-scene31 and75 layout checks per size PASS; launcher ownership6 tests | Original partial/failing runs preserved; final active960/results1280 spectator PNGs directly inspected | Independent two-client Meridian Leave/rejoin/results/host restart; final read-only connection1776 snapshots, zero gameplay/config/start requests PASS | Fixed-view active spectator and bounded lifecycle accepted; first exported full flow hit clean-log focus errors, triage OPEN |
...
| Common launcher | Routing regressions cover nine routes/eight scenes; standalone pairs checked against locked catalog; explicit lobby endpoint ownership tested | Original startups and expanded release-scene captures retained | Owned lobby and external-server client startup/close/interrupt PASS in initial export run; whole-run X11 verifier race retained | One command family; external authority is never created/closed by guest launcher; interactive mode has no harness deadline |
| Native CI | Official pinned archive/hash, fresh npm install and GLB generation checked | Hosted successes and original audio failure retained | Ubuntu GitHub Actions run `35690697596` passes all80 gates at `b2eef7c`; complete downloaded summary retained | Current summary survives artifact cap; graphical/package/full-gameplay evidence remains separate |
| Linux prototype package | Official toolchain/ws integrity and source closure validated; eight release scenes/nine routes/nine maps present, test/probe resources absent | Initial X11 verifier failure preserved; real destroyed-window regression fails old/passes repaired | Attempt02 passes all startup/cleanup cases including owned/external lobby and external-server preservation; bytes unchanged | Editor-free startup accepted; first exported full lobby flow clean-log FAIL under focus triage; no blanket exported gameplay claim |
| Full verifier | All **80** implemented gates pass after lobby/spectator/world/Payload integration | Original failures retained and corrected | Complete rerun includes strict spectator context, source/transport ownership and new guidance contracts | See `reports/verification.json`; graphical/playable gates separate |
...
- The combined 80-gate verifier passes with rocket presentation, sports
