...
| Native CI | Official pinned archive/hash, fresh npm install and GLB generation checked | Hosted successes and original audio failure retained | Ubuntu GitHub Actions run `35682361375` passes all 63 gates at `e76e3b2`, including package-routing validation | Fresh-checkout automation accepted; actual Linux export/artifact play verified separately |
| Linux prototype package | Official toolchain/ws integrity and source closure validated; five release scenes/nine maps present, test/probe resources absent | Independent archive/hash manifest, build logs and release-client images retained | Rebuilt at `8a58c97`: fresh Node-only setup/combat/world and X11 C tactical-panel opening; window/Ctrl+C/crash/missing-executable cleanup PASS, bytes unchanged | Editor-free local Linux startup accepted; not a public release or exported sports-completion claim |
| Full verifier | All **65** implemented gates pass after sports/world-command integration | Original failures and earlier pass reports retained | Complete rerun on `8a58c97` runtime plus coaching/world-command gates passes | See `reports/verification.json`; graphical/playable gates separate |
...
- The combined 65-gate verifier passes with rocket presentation, sports
