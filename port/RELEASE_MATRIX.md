...
| Common launcher | Routing regressions cover eight experience routes; standalone pairs checked against locked catalog | Original startups and expanded release-scene captures retained | Exported Arms Race/combat/world/zones/vehicles/both sports startup and cleanup PASS | One command family; interactive mode has no harness deadline |
...
| Linux prototype package | Official toolchain/ws integrity and source closure validated; eight release scenes/nine maps present, test/probe resources absent | Independent archive/hash manifest, build logs and release-client images retained | Arms Race/bearing rebuild: fresh Node-only setup/combat/world/zones/combined-arms/Arms Race/race/soccer startup and X11 C panel opening; cleanup PASS, bytes unchanged | Editor-free local Linux startup accepted; not a public release or exported full-gameplay claim |
| Full verifier | All **73** implemented gates pass after Arms Race/bearing integration | Original failures retained and corrected | Complete rerun includes Arms Race source38/native55 and sports camera-projection896 | See `reports/verification.json`; graphical/playable gates separate |
...
- The combined 73-gate verifier passes with rocket presentation, sports
