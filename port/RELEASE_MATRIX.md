...
| Native CI | Official pinned archive/hash, fresh npm install and GLB generation checked | Hosted successes and original audio failure retained | Ubuntu GitHub Actions run `35679051417` passes all 57 gates at `bfcc9ed`, including fresh repaired GLB and objective progression | Fresh-checkout automation accepted; graphical/hardware gameplay separate |
| Full verifier | All **59** implemented gates pass after co-op economy and soccer guidance integration | Original failures and earlier pass reports retained | Complete rerun at `4eb0e36` plus economy/soccer gates passes | See `reports/verification.json`; graphical/playable gates separate |
...
| Asterion Relay | LATTICE (`cocs`, `cocs-coop`) | Native Map/List/HOLD/PvP Fighter and co-op REINFORCE independently pass; traversal and full objective gameplay open |
| Monsoon Foundry | LATTICE (`cocs`, `cocs-coop`) | Native Map/List/HOLD/PvP Fighter and co-op REINFORCE independently pass; traversal and full objective gameplay open |
...
- The combined 59-gate verifier passes with rocket presentation, sports
