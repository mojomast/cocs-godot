...
| Combined-arms Puma | 43 native controls/lease checks and 8 replay/negative tests pass | Delivery source-correlated mount/26.833m drive/brake/exit/3.262m infantry, plus retained failures | Independent exported Sunscar startup PASS; independent full-route reviewer running | Driving slice integrated; secondary chassis are visual/exit-only previews, broader vehicle combat and human camera review OPEN |
...
| LATTICE world commands | 27 boundary/consent checks pass | Independent 960×640 / 1280×800 command panels directly inspected | At `511bb8d`, 8/8 focused cases PASS: same-socket HOLD, one PvP Fighter / 12 FLUX / +1 actor, co-op closed-window blocking and both old traversal runs | In-world panel and safe return controls accepted; exported X11 C opens the real panel; new live in-world co-op purchase/full strategy rounds OPEN |
...
| Common launcher | Routing regressions cover seven experience routes; standalone pairs checked against locked catalog | Original startups and expanded release-scene captures retained | Package defaults for zones/combined arms plus existing sports/combat/world launch and clean up | One command family; interactive mode has no harness deadline |
| Native CI | Official pinned archive/hash, fresh npm install and GLB generation checked | Hosted successes and original audio failure retained | Ubuntu GitHub Actions run `35684992377` passes all 65 gates at `83e4aff` | Fresh-checkout automation accepted; newer local gates and Linux export verified separately |
| Linux prototype package | Official toolchain/ws integrity and source closure validated; seven release scenes/nine maps present, test/probe resources absent | Independent archive/hash manifest, build logs and release-client images retained | Zone/vehicle rebuild: fresh Node-only setup/combat/world/zones/combined-arms/race/soccer startup and X11 C panel opening; cleanup PASS, bytes unchanged | Editor-free local Linux startup accepted; not a public release or exported full-gameplay claim |
| Full verifier | All **69** implemented gates pass after zone/vehicle integration | First combined replay-fixture selection failure retained and corrected | Complete rerun passes including zone routing/adapter and vehicle replay/controls | See `reports/verification.json`; graphical/playable gates separate |
...
- The combined 69-gate verifier passes with rocket presentation, sports
