...
| Multiplayer lobby / spectator | Lobby33, strict notice-context66, lead actual-scene31 and75 layout checks per size PASS; launcher ownership6 tests | Original partial/failing runs preserved; release-popup comparison reproduced14 errors vs debug0 with identical72 actions | Independent two-client Meridian Leave/rejoin/results/host restart; final read-only connection1776 snapshots, zero gameplay/config/start requests PASS | Fixed-view spectator/bounded lifecycle accepted; full exported play HOLD, popup-free selector repair underway |
