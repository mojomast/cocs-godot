...
| Native CI | Official pinned archive/hash, fresh npm install and GLB generation checked | Hosted successes and original audio failure retained | Ubuntu GitHub Actions run `35685825939` passes the 69-gate suite at `5445295`; console retained, summary omitted by old artifact cap | Collector now prioritizes current summary; cap regression passes locally, hosted follow-up pending |
...
| Full verifier | All **70** implemented gates pass after setup-copy/receipt/CI fixes | First combined replay-fixture selection failure retained and corrected | Complete rerun passes including artifact-cap regression and stronger vehicle replay | See `reports/verification.json`; graphical/playable gates separate |
...
- The combined 70-gate verifier passes with rocket presentation, sports
