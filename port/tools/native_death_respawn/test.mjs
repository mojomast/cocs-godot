...
test('genuine earlier runtime failure stays failed with premature zero-HP alive state',()=>{
 ...
 assert.equal(r.status,'FAIL');assert.equal(r.criteria.strictLifecycleContinuity,false);assert.equal(r.criteria.deadLifecycleGated,false);assert.equal(r.criteria.cameraReseed,false);
