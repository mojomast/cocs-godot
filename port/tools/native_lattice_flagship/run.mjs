...
const maxRows=12000,maxBytes=16*1024*1024;let rows=0,bytes=0,latestSourceRevision=null,lastSnapshotAt=-1000,lastSnapshotRevision=null;
...
try {
 ...
 function record(direction,frame) {
   ...
   if(direction==='recipient'&&frame.type==='snapshot'){
    ...
    if(rev===lastSnapshotRevision&&Date.now()-start-lastSnapshotAt<1000)return;
    lastSnapshotRevision=rev;lastSnapshotAt=Date.now()-start;
   }
