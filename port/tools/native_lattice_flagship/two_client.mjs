...
async function main(){
 ...
 const dir=join(root,'port/native-lattice/evidence/flagship',`two-client-${Date.now()}-${randomUUID()}`);mkdirSync(dir,{recursive:true});
  const manifest={schema_version:1,evidence_class:engineDriver?'engine-driven-two-native-smoke':'ordinary-wire-two-native',source_commit:lock.source_commit,port_commit:execFileSync('git',['rev-parse','HEAD'],{cwd:root,encoding:'utf8'}).trim(),map:args.map,mode:args.mode,input_method:engineDriver?'engine-scripted Start and ordinary source inputs; no OS/human input':'human clicks host Start / restart in native window; Enter in terminal only records operator acknowledgement and never sends protocol data',start_gate:'recipient-observed guest lobby presence and two distinct peers before start; then recipient-observed source start and distinct assigned actors for both clients',guest_start_security:'UNVERIFIED: recorded guest outgoing frames are inspectable but absence alone does not prove authorization/security',hashes:Object.fromEntries(['godot/lattice/world_demo.gd','godot/lattice/session_options.gd','port/tools/native_lattice_flagship/two_client.mjs',...(engineDriver?['godot/tests/lattice/probe_pair_flagship.gd']:[])].map(p=>[p,createHash('sha256').update(readFileSync(join(root,p))).digest('hex')]))};
  ...
  let rows=0,bytes=0;const sockets={host:[],guest:[]},reports={host:[],guest:[]},children=[];let game,runtime,serverClosed=false,childrenWaited=false,reason='blocked-awaiting-human-click',error=null,observation=null;
 ...
 try{game=createGameServer({historyPath:null,progressionPath:null});await new Promise((ok,no)=>{game.server.once('error',no);game.server.listen(0,'127.0.0.1',ok)});const endpoint=`ws://127.0.0.1:${game.server.address().port}`;manifest.endpoint=endpoint;writeFileSync(join(dir,'manifest.json'),JSON.stringify(manifest,null,2));
  ...
  runtime=join(dir,'xdg');mkdirSync(runtime);const env={...process.env,XDG_DATA_HOME:join(runtime,'data'),XDG_CONFIG_HOME:join(runtime,'config'),XDG_CACHE_HOME:join(runtime,'cache')};for(const p of [env.XDG_DATA_HOME,env.XDG_CONFIG_HOME,env.XDG_CACHE_HOME])mkdirSync(p);
   ...
   const launch=(who,extra=[])=>{const argv=engineDriver?['--audio-driver','Dummy','--headless','--path','godot','--script','res://tests/lattice/probe_pair_flagship.gd','--',`--endpoint=${endpoint}`,`--map=${args.map}`,`--mode=${args.mode}`,`--pair-role=${who}`,`--start-file=${startFile}`,`--stop-file=${stopFile}`,'--native-trace',...extra]:['--audio-driver','Dummy','--path','godot','res://lattice/world_demo.tscn','--',`--endpoint=${endpoint}`,`--map=${args.map}`,`--mode=${args.mode}`,'--native-trace',...extra];const c=spawn(process.env.GODOT_BIN,argv,{cwd:root,env,stdio:['ignore','pipe','pipe'],detached:true});children.push(c);for(const stream of [c.stdout,c.stderr])createInterface({input:stream}).on('line',l=>{logs.write(`${who}: ${l.slice(0,4000)}\n`);if(l.startsWith('LATTICE_PAIR_PROBE '))try{reports[who].push(JSON.parse(l.slice(19)))}catch{};if(l.startsWith('PORT_NATIVE_TRACE '))try{traces.write(JSON.stringify({client:who,...JSON.parse(l.slice(18))})+'\n')}catch{}});return c};
   ...
   if(engineDriver){await until(()=>reports.host.some(r=>r.status==='active'&&r.input_sent>=45)&&reports.guest.some(r=>r.status==='active'&&r.input_sent>=45),30000,controller.signal);observation=evaluatePair(sockets);if(!observation.two_distinct_peers||!observation.two_distinct_actors)throw Error('Two simultaneously connected source actors not observed');writeFileSync(stopFile,'pair observation complete\n')}
   reason=engineDriver?'engine-driven-two-client-observed':'source-start-observed-both';if(!engineDriver)await new Promise(r=>setTimeout(r,5000));
  ...
  await Promise.all(children.map(c=>c.exitCode!==null||c.signalCode!==null?Promise.resolve():new Promise(ok=>{c.once('exit',ok);setTimeout(()=>{try{process.kill(-c.pid,'SIGKILL')}catch{}},3000)})));childrenWaited=true;
   ...
   const evaluation=observation??evaluatePair(sockets);writeFileSync(join(dir,'results.json'),JSON.stringify({status:reason==='source-start-observed-both'?'OBSERVED':reason==='engine-driven-two-client-observed'?'ENGINE_SMOKE_OBSERVED':'BLOCKED',reason,evaluation,reports,guest_start_authorization:'UNVERIFIED',guest_outgoing:sockets.guest.filter(x=>x.direction==='client'),cleanup:{children_waited:childrenWaited,server_closed:serverClosed,temp_removed:!runtime||!existsSync(runtime)},error},null,2));
