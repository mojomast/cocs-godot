...
export function analyze(stdout,wire){
 const records=[],associated=[],observations=[];let monotonic=-1,previous=null;
 for(const [i,line] of stdout.split('\n').entries()){
  ...
  }else if(line.startsWith('HEALTH_OBSERVE '))observations.push(JSON.parse(line.slice(15)));
 }
 ...
 const matched=natives.map((r,i)=>{
  ...
  const hp=/\| HP ([\d.eE+-]+) \| Armor ([\d.eE+-]+)/.exec(o.hud);assert.ok(hp,'native HUD format');assert.ok(near(Number(hp[1]),w.actor.health));assert.ok(near(Number(hp[2]),w.actor.armor));assert.ok(o.label.includes(o.hud),'display label contains current authoritative HUD');
  return {r,o,w,p};
 });
 ...
 const hurtUI=observations.find(o=>o.event==='frame'&&o.hurts>0&&o.hurt_remaining>0&&o.combat_text.includes('TAKING DAMAGE')&&o.ui_text.includes('TAKING DAMAGE'));
 assert.ok(hurtUI,'actual combat Label displayed TAKING DAMAGE');
 ...
 return {status:'PASS',actor:wire.actor,attacker:wire.attackerActor,roundRevision:wire.starts[0].roundRevision,completionProven:false,
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
 try{const dir=process.argv[2],result=analyze(gunzipSync(readFileSync(`${dir}/native.stdout.log.gz`)).toString(),JSON.parse(gunzipSync(readFileSync(`${dir}/wire.json.gz`))));console.log(JSON.stringify(result,null,2));}
