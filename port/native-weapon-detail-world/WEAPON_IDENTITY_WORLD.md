...
| 3 Scattergun | source wide breech block, top rib over twin bores, breech extractor below | break-action extractor plate, right-side shell carrier with three spare rims | twin bores with muzzle clamp band, per-bore choke rings and side lugs | wide stock with recoil pad, shell loops on the left, butt plate | source iron sights plus a brass bead on the top rib | amber shell rims and carrier; emissive chamber witness ring |
| 4 Plasma Driver | source rounded vented chamber, cradle rails, power pack | energy cell: twin cell tubes, emissive plasma window, heavy latch | flared focus cone with three standoff prongs, crown ring and a glowing core | vented frame stock (two side rails per flank), heat-shield plate, butt plate | source iron sights with a heat shroud around the front post base | blue emissive chamber window and barrel strip; ribbed heat-sink rings |
| 5 Grenade Launcher | source revolver frame, quadrant arc on the left flank | revolver drum: six flutes proud of the drum, front and rear hub caps, index marks, charge window | heaved bored muzzle with two collars, lateral ports and a front tube | compact stock with recoil pad, thumb rest, butt plate; trigger group moved behind the drum | source iron sights with a ladder leaf and the left-flank quadrant arc | red-orange emissive drum index window; angled drum flutes |
| 6 Shock Beam | source casing with a capacitor bank and a capacitor drum above the receiver | energy cell: capacitor plates, cell housing, vent slats | twin fork prongs with discharge blocks and a tuning bridge | open frame stock, insulated cheek plate | source iron sights on a short rail | cyan emissive arc bar between the prongs, barrel strip and discharge tips; insulator rings and exposed coil discs |
...
## Features named by the shared document but not built here

Named in `WEAPON_IDENTITY.md` and deliberately left out at the third-person budget
(each is a sub-feature inside a channel this kit answers with other geometry, not a
missing channel):

| weapon | named sub-feature not built | why |
|---|---|---|
| 0 Pulse Rifle | squared front trunnion, paired side plates, magwell flare | the rail, port plates and stepped fins already carry the receiver read; the magazine catch, witness slots and floorplate carry the feed read |
| 1 Rocket Launcher | separate flared rear venturi tube/flange | the source world body's rear end carries the shoulder rest and strap; the emissive dot ring marks the venturi instead |
| 3 Scattergun | barrel-selector bar, comb block | the wide breech, top rib, bead, extractor, carrier and recoil pad already read; a selector lever is a viewmodel-distance part |
| 4 Plasma Driver | grip fin stack | the grip is inside the closed palm; fins there would be invisible or would intersect it |
| 5 Grenade Launcher | top strap | the rail, ladder leaf and quadrant arc already break the frame's outline |
| 6 Shock Beam | stock capacitor block | the capacitor drum sits above the receiver, where it reads |
| 8 Marksman Rifle | funnel magwell | the catch, floorplate and witness slots carry the feed read; a funnel is a viewmodel-distance part |
