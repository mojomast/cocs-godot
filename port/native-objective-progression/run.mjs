...
const seconds=map==='tidal-citadel'?60:90, deadline=150000;
const out=resolve('port/native-objective-progression/evidence',randomUUID());mkdirSync(out,{recursive:true});
const temp=mkdtempSync('/tmp/opencode/objective-progression-runtime-');
...
const files=['godot/objectives/demo.gd','godot/objectives/renderer.gd','godot/objectives/hud.gd','godot/tests/objectives/progression_live.gd','godot/world/session.gd','godot/net/client.gd','game/core.mjs','game/payload.mjs','server/room.mjs','server/game-server.mjs','port/native-objective-progression/run.mjs','port/native-objective-progression/peer.mjs','port/native-objective-progression/validate.mjs'];
...
const children=[];
function witness(record){const line=JSON.stringify({wall:Date.now(),...record});bytes+=line.length;if(bytes>48*1024*1024)throw Error('Evidence cap');wire.push(line);}
...
try {
 ...
 child=tracked(binary,['--audio-driver','Dummy','--resolution',args.includes('--small')?'960x640':'1280x800','--path','godot','res://tests/objectives/progression_live.tscn','--',`--map=${map}`,`--endpoint=${endpoint}`,'--objective-evidence','--objective-peer',`--round-seconds=${seconds}`,`--screenshot=${out}/gameplay.png`],{env,stdio:['ignore','pipe','pipe']});
 child.stdout.on('data',b=>{stdout+=b;if(stdout.length>32*1024*1024){reason='stdout cap';child.kill('SIGTERM');}});child.stderr.on('data',b=>stderr+=b);
 ...
 if(result.code===0&&!/SCRIPT ERROR|Parse Error|ERROR:/.test(stdout+stderr)){
  ...
  writeFileSync(resolve(out,'validation.json'),JSON.stringify(validation,null,2));exit=0;reason='source-correlated progression passed';
 ...
 const archive={};for(const [name,text]of Object.entries(logs)){const raw=Buffer.from(text);const packed=gzipSync(raw);writeFileSync(resolve(out,name+'.gz'),packed);archive[name]={bytes:raw.length,sha256:createHash('sha256').update(raw).digest('hex')};}
 ...
 writeFileSync(resolve(out,'summary.json'),JSON.stringify({map,seconds,deadline,base:'d498479',source:lock.source_commit,runtimeHashes,exit,reason,normalRate:true,nativeCompletionProven:false,cleanup,serverClosed:!game?.server.listening,sockets:game?.wss.clients.size??0,temporaryTreeRemoved:!existsSync(temp)},null,2));
