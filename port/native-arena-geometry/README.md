...
The 2026-09-22 trap-fix lane changed the compiler's movement-wall rules on top of the retained geometry (see `port/native-arena-trap-fix/REPORT.md`): one-sided burial and terrace handling of wall bands, raised band bottoms over lower floors, walkable-topped DM guard rails, and a `dm_walkable` cap on the adapted guard barriers. Those edits change every `geometryHash`; the values below and in `verification.json` are the re-accepted delivered hashes of this revision.
