...
const output=resolve('port/native-entity-presentation');
...
try{
  ...
  const fixture=launch(binary,[...common,'--script',resolve(output,'render_fixture.gd'),'--','--output='+resolve(output,'composition.png')],'render');
  ...
  const native=launch(binary,[...common,'--script',resolve(output,'capture_session.gd'),'--',`--endpoint=ws://127.0.0.1:${game.server.address().port}`,'--output='+output],'native');
