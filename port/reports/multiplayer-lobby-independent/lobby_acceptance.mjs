...
try{
 ...
 await fill(guest,'player_name','Independent Guest');await click(guest,'role');await key(guest,4194322);await key(guest,4194309);await fill(guest,'room',summary.room);await click(guest,'connect_button');await wait(()=>latest(guest)?.phase===11,10000,'guest joined');await wait(()=>latest(host)?.roster.includes('Independent Guest'),4000,'authoritative two-human roster');await capture(guest,'04-guest-roster');
