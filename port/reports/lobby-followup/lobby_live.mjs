...
const startup=process.argv.includes('--ember-startup'),map=startup?'ember-crucible':'meridian-exchange',mode=startup?'rockets':'teamdeathmatch';
const out=resolve('port/reports/lobby-followup',startup?'ember-startup':'meridian-tdm');
assert.ok(!existsSync(out),'Preserve existing captures');mkdirSync(out);
const temp=mkdtempSync('/tmp/opencode/lobby-followup-live-'),children=[],wire=[],actions=[];
...
function spawnOwned(name,cmd,args,options){const p=spawn(cmd,args,options);p.name=name;p.text='';p.err='';p.samples=[];p.done=new Promise((res,rej)=>{p.once('error',rej);p.once('close',res);});children.push(p);if(p.stdout){let buffer='';p.stdout.on('data',b=>{p.text+=b;buffer+=b;let i;while((i=buffer.indexOf('\n'))>=0){const line=buffer.slice(0,i);buffer=buffer.slice(i+1);if(line.startsWith('LOBBY_SAMPLE '))p.samples.push(JSON.parse(line.slice(13)));}if(p.text.length>32*1024*1024)p.kill('SIGTERM');});}p.stderr?.on('data',b=>p.err+=b);return p;}
const latest=p=>p.samples.at(-1);
async function wait(pred,ms,label){await until(()=>{if(expired)throw Error('180s deadline');for(const p of [host,guest])if(p&&(p.exitCode!==null||p.signalCode!==null))throw Error(`${p.name} exited: ${p.err}`);return pred();},ms,label);}
...
try{
 ...
 timer=setTimeout(()=>{expired=true;for(const p of children)p.kill('SIGTERM');},180000);
