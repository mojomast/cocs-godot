...
func witness(event: String) -> void:
	...
	if is_instance_valid(session) and is_instance_valid(session.lobby_menu):
		for name: String in ["role","maps","modes"]:
			var item: Control = session.lobby_menu.get(name)
			choices[name] = {"visible":item.is_visible_in_tree(),"focused":item.has_focus(),"selected":item.selected,"disabled":item.disabled}
	var connections := {}
	...
	print("EXPORT_WITNESS ",JSON.stringify({"wall":Time.get_unix_time_from_system()*1000,"ticks":Time.get_ticks_usec(),"event":event,"stage":stage,"command":last_command,"root_focused":root.has_focus(),"choices":choices,"connections":connections,"lobby_popup_count":popup_count(session.lobby_menu)}))
const KEYS := {"W":KEY_W,"A":KEY_A,"Escape":KEY_ESCAPE,"Enter":KEY_ENTER,"Left":KEY_LEFT,"Right":KEY_RIGHT,"Tab":KEY_TAB}
...
func key(code: int, pressed: bool, unicode_value := 0, ctrl := false, shift := false) -> void:
...
func command(c: Dictionary) -> void:
	...
	match c.op:
		...
		"key": key(KEYS[c.key],c.pressed,0,c.get("ctrl",false),c.get("shift",false))
		"mouse":
...
func _process(delta: float) -> bool:
	...
	for name: String in ["endpoint","player_name","room","role","maps","modes","connect_button","start_button","back_button","leave_button","restart_button"]:
		...
		if name in ["role","maps","modes"]:
			ui[name].selected = control.selected
			ui[name].disabled = control.disabled
			ui[name].focused = control.has_focus()
			for part: String in ["previous","next","current_label"]:
				var child: Control = control.get(part)
				ui[name+"_"+part] = {"rect":rect(child),"visible":child.is_visible_in_tree(),"in_form":true,"disabled":control.disabled,"focused":child.has_focus(),"text":child.get("text")}
			ui[name].items = []
