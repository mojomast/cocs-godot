...
function spawnOwned(name,cmd,args,options){const p=spawn(cmd,args,options);p.name=name;p.text='';p.err='';p.samples=[];p.spectators=[];p.done=new Promise((res,rej)=>{p.once('error',rej);p.once('close',res);});children.push(p);if(p.stdout){let buffer='';p.stdout.on('data',b=>{p.text+=b;buffer+=b;let i;while((i=buffer.indexOf('\n'))>=0){const line=buffer.slice(0,i);buffer=buffer.slice(i+1);if(line.startsWith('LOBBY_SAMPLE '))p.samples.push(JSON.parse(line.slice(13)));if(line.startsWith('SPECTATOR_SAMPLE '))p.spectators.push(JSON.parse(line.slice(17)));}if(p.text.length>32*1024*1024)p.kill('SIGTERM');});}p.stderr?.on('data',b=>p.err+=b);return p;}
...
async function cmd(p,c){c={id:++commandId,...c};actions.push({wall:Date.now(),client:p.name,...c});writeFileSync(p.inbox+'.next',JSON.stringify(c));renameSync(p.inbox+'.next',p.inbox);await wait(()=>latest(p)?.command===c.id,3000,'command '+c.op);}
async function focus(p){await cmd(p,{op:'focus'});await wait(()=>latest(p)?.focused,3000,'focus');}
...
async function capture(p,name){await cmd(p,{op:'capture',name});await wait(()=>existsSync(resolve(out,name+'.png')),3000,'PNG '+name);summary.milestones.push({name,wall:Date.now(),client:p.name,sample:latest(p),spectator:p.spectators.at(-1)});}
...
function readonly(label){const s=guest.spectators.at(-1);pass(label+' validated spectator',s?.spectating&&s.actor===-1&&s.ack===0&&s.input_seq===0&&!s.pose&&!s.captured&&!s.eligible);pass(label+' explicit read-only UI',s.title.includes('SPECTAT')&&!s.detail.includes('Local actor absent')&&!s.controls_visible&&s.score==='SPECTATOR');}
async function attemptControls(label){await focus(guest);await cmd(guest,{op:'key',key:'W',pressed:true});await cmd(guest,{op:'mouse',x:guest.width/2,y:guest.height/2,pressed:true});await sleep(600);readonly(label);await cmd(guest,{op:'mouse',x:guest.width/2,y:guest.height/2,pressed:false});await cmd(guest,{op:'key',key:'W',pressed:false});}
try{
 ...
 timer=setTimeout(()=>{expired=true;for(const p of children)p.kill('SIGTERM');},120000);
 ...
 await gameplay(guest,'07-before-leave');
 summary.originalGuestPeer=latest(guest).peer;summary.originalGuestActor=latest(guest).actor;
 await leave(guest,'08-player-leave');
 await click(guest,'connect_button');await wait(()=>latest(guest)?.phase===3&&guest.spectators.at(-1)?.spectating&&guest.spectators.at(-1)?.snapshots>=5,10000,'connected spectator receives state');
 summary.spectatorPeer=latest(guest).peer;readonly('active rejoin');await capture(guest,'09-spectator-1280');layout(guest,'spectator 1280');
 await attemptControls('held controls before restart');await gameplay(host,'10-host-moves-for-spectator');
 await capture(guest,'11-spectator-moving-1280');readonly('moving state');
 await cmd(guest,{op:'resize',width:960,height:640});guest.width=960;guest.height=640;await wait(()=>latest(guest)?.viewport[0]===960,3000,'spectator resize');
 await capture(guest,'12-spectator-960');layout(guest,'spectator 960');
 await cmd(guest,{op:'key',key:'Tab',pressed:true});await wait(()=>latest(guest)?.layout.board.visible,3000,'spectator scoreboard');await capture(guest,'13-spectator-scoreboard-960');layout(guest,'spectator scoreboard');await cmd(guest,{op:'key',key:'Tab',pressed:false});
 await wait(()=>latest(host)?.phase===4&&latest(guest)?.phase===4,70000,'natural 60s results for both');readonly('results');await capture(guest,'14-spectator-results-960');layout(guest,'spectator results 960');
 await cmd(guest,{op:'resize',width:1280,height:800});guest.width=1280;guest.height=800;await wait(()=>latest(guest)?.viewport[0]===1280,3000,'results resize');await capture(guest,'15-spectator-results-1280');layout(guest,'spectator results 1280');
 const before=wire.filter(f=>f.type==='received'&&f.frame.type==='start').length;await focus(guest);await key(guest,'Enter');await sleep(400);pass('spectator cannot restart',wire.filter(f=>f.type==='received'&&f.frame.type==='start').length===before&&latest(guest).phase===4&&!latest(guest).ui.restart_button.visible);
 await click(host,'restart_button');await wait(()=>latest(host)?.revision===2&&latest(guest)?.revision===2&&guest.spectators.at(-1)?.revision===2&&guest.spectators.at(-1)?.snapshots>=5,10000,'authoritative restart received');readonly('host restart preserves spectator');pass('same spectator connection through restart',latest(guest).peer===summary.spectatorPeer);await capture(guest,'16-spectator-restarted');
 await attemptControls('held controls after restart');await gameplay(host,'17-host-round-two');await capture(guest,'18-spectator-round-two');readonly('new round state');
 ...
 pass('spectator queues zero input or host commands',received.filter(f=>f.recipient===2).every(f=>['join','leave'].includes(f.frame.type)));
 const results=wire.filter(f=>[0,2].includes(f.recipient)&&f.type==='results');pass('natural source clock for host and spectator',results.length===2&&results.every(f=>f.state.time>=60&&f.state.time<61));summary.sourceResultsTime=results[0].state.time;
 pass('no native script errors',[host,guest].every(p=>!/SCRIPT ERROR|Parse Error|ERROR:/.test(p.text+p.err)));summary.status='PASS';
