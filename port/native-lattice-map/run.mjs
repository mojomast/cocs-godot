// Adapted from native-lattice-physical/run.mjs, scoped map observer + witness.
// Normal-rate owned loopback server; only recipient allow-listed fields retained.
...
game.wss.on('connection', ws => {
 ...
 ws.send = function(data, ...args) {
  ...
  if (frame.type === 'snapshot' && frame.state.cocs) {
   ...
   latest = {direction:'out', type:'recipient-snapshot', peer, actor, team, seq:frame.seq,
    ...
    nodes:board.nodes.map(node => pick(node, ['id','label','x','z','owner','live','contested'])),
    cards:(board.cards ?? []).filter(card => String(card.peerId) === String(peer)).map(card => pick(card, ['id','actorId','peerId','team','verb','target','state','accepted','ok','reason']))};
 ...
 ws.prependListener('message', data => {
  ...
  if (['order','economy'].includes(frame.type)) record({direction:'in', peer, actor,
   ...
   priorRecipient:latest ? {seq:latest.seq, team:latest.team, nodes:latest.nodes} : null});
 });
});
...
try {
 ...
 timer = setTimeout(() => {console.error('Map attempt 90s deadline'); stop();}, 90000);
 ...
 const checks = {
  ...
  holdExactlyOnce:hold.length === 1 && action.verb === 'HOLD' && action.target === `front-${action.priorRecipient.team}`,
  publicTarget:hold.length === 1 && action.priorRecipient.nodes.some(node => node.id === action.target && Number.isFinite(node.x) && Number.isFinite(node.z)),
  holdSameTargetRunning:hold.length === 1 && snapshots.some(snapshot => snapshot.peer === action.peer && snapshot.actor === action.actor && snapshot.round === action.roundRev && snapshot.cards.some(card => card.id === action.cardId && card.target === action.target && card.actorId === action.actor && String(card.peerId) === String(action.peer) && card.state === 'running' && card.ok === true && card.accepted === true)),
  ...
  noImplicitPurchase:inbound.every(action => action.type !== 'economy'),
