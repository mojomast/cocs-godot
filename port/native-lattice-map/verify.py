"""Every attempt is retained; private Xvfb and ephemeral server per live case."""
import json
import os
...
out = Path('port/native-lattice-map/evidence') / str(time.time_ns())
...
cases = [
    ...
    *[(name, [binary, '--headless', '--path', 'godot', '--script', f'res://tests/lattice/{name}.gd']) for name in ['adapter', 'ui', 'map_view']],
]
for lane, helper in [('map', 'native-lattice-map'), ('original', 'native-lattice-physical')]:
    for map_id, mode, size in [('asterion-relay', 'cocs', '960x640'), ('monsoon-foundry', 'cocs', '1280x800'), ('asterion-relay', 'cocs-coop', '960x640')]:
        ...
        cases.append((name, ['xvfb-run', '-a', '-s', '-screen 0 1400x1000x24 -nolisten tcp -nolisten unix',
                            'node', f'port/{helper}/run.mjs', f'--map={map_id}', f'--mode={mode}',
                            f'--size={size}', f'--output={(out / name).resolve()}']))
results = []
for name, command in cases:
    with tempfile.TemporaryDirectory(prefix='map-check-', dir='/tmp/opencode') as runtime:
        env = dict(os.environ)
