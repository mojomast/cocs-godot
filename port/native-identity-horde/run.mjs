...
const out = resolve(ROOT, `port/native-identity-horde/evidence/${stamp}-${scenario}`);
...
try {
  ...
  const argv = ['--audio-driver', 'Dummy', ...(rendering ? ['--rendering-method', rendering] : []),
    ...
    'res://tests/horde/identity_live.tscn', '--',
    `--map=${IDENTITY_HORDE_MAP}`, `--endpoint=${endpoint}`, `--waves=${waves}`,
  ...
  writeFileSync(resolve(out, 'launch.json'), JSON.stringify({
    ...
    argv, scenario, map: IDENTITY_HORDE_MAP, waves, resolution, rendering: rendering || 'engine-default',
    ...
    scope: 'identity-family Horde on Nacre Engine',
  }, null, 2));
  ...
  const summary = {exit, reason, scenario, map: IDENTITY_HORDE_MAP, waves, resolution, rendering: rendering || 'engine-default',
