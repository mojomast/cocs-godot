...
const seconds=passing?60:180,deadline=passing?110000:235000;
...
files.push('godot/tests/objectives/completion_pass.gd','port/native-objective-completion/pass-peer.mjs','port/native-objective-completion/pass-validate.mjs','port/native-objective-completion/inputs.mjs');
const runtimeHashes=Object.fromEntries(files.map(p=>[p,createHash('sha256').update(readFileSync(p)).digest('hex')]));
let game,child,guest,friendly,timer,exit=1,reason='not started',stdout='',stderr='',wire=[],bytes=0,connection=0;
const children=[];
...
try{
 ...
 game.wss.on('connection',socket=>{
  ...
  const send=socket.send;socket.send=function(data,...rest){
   ...
   if(f.type==='welcome'&&id===0)queueMicrotask(()=>{guest=passing?passPeer(endpoint,f.roomId,false,witness):opposingPeer(endpoint,f.roomId,map,witness);});
   if(f.type==='welcome'&&id===1&&passing)queueMicrotask(()=>{friendly=passPeer(endpoint,f.roomId,true,witness);});
   if(['welcome','lobby','start','results','events'].includes(f.type))witness({connection:id,round,...(f.type==='results'?{type:f.type,state:compact(f.state)}:f.type==='welcome'?{type:f.type,v:f.v,peerId:f.peerId,roomId:f.roomId,host:f.host,spectate:f.spectate}:f)});
 ...
 if(result.code===0&&!/SCRIPT ERROR|Parse Error|ERROR:/.test(stdout+stderr)){
  const validation=(passing?validatePass:validate)(wire.map(JSON.parse),stdout,map,seconds);
 ...
 clearTimeout(timer);guest?.terminate();friendly?.terminate();
