...
See [PLAY.md](PLAY.md) for all ten experience routes and direct exported-scene
commands, including `--experience=zones` and `--experience=combined-arms`.
Current commands and controls are also in the root
[README](../../README.md#zone-control-and-combined-arms). The ten-route/nine-scene
rebuild is verified in [the latest integration report](../reports/linux-horde-independent/README.md).
The prior popup-free package's clean two-client lobby flow remains separately
documented in [its report](../reports/linux-popup-free-independent/README.md).
...
Horde adds a distinct local-only entry point. The discovered union contains
**84 immutable source modules** and **2 port-owned adapter modules**:
`port/native-horde/authority.mjs` and `input-buffer.mjs`. Horde reaches73 of those
source modules; its source closure is established by import traversal. The
builder compares source bytes to the pinned source commit, adapter bytes to the
reviewed port HEAD, and writes separate `source_runtime_sha256` and
`port_adapter_sha256` inventories. The adapter modules ship under
`runtime/port/native-horde/`, preserving relative imports. No observer, event
oracle, validator or test module enters that runtime closure. Public Room's
Horde rejection remains unchanged.

Only `--experience=horde` creates that local adapter, and both launchers require
its exact readiness identity (`cocs-local-horde`, transport1, localOnly true,
allocated port). Ordinary owned routes still use the original server factory;
an explicit external-lobby endpoint constructs neither authority. Invalid CLI
options fail before construction. Horde's common route uses the native default
ten-wave target without exposing wave/endless/upgrade controls.
...
- All nine scene paths are exported: combat, sports, objectives, LATTICE board,
  LATTICE world, zones, combined arms, Arms Race and Horde. Routing is package-local and does
...
executable/PCK. It verifies all nine semantic maps, all nine compiled scenes,
