// Passive public-wire receipts only. Owned loopback port 0, normal source clock.
...
const evidence={clock:'default normal-rate server; no state writes',inputs:[],samples:[],events:[],results:[],configs:[],starts:0};
const since=Date.now();
let lastSample=0, actor=null;
const sample=f=>({wallMs:Date.now()-since,seq:f.seq,ack:f.acks?.[actor],time:f.state.time,actor,over:f.state.over,overReason:f.state.overReason,roles:f.state.actors?.map(a=>({id:a.id,team:a.team,bot:Boolean(a.bot)})),vehicle:f.state.vehicles?.find(v=>v.driver===actor),race:f.state.race});
game.wss.on('connection',socket=>{
 socket.on('message',raw=>{const f=JSON.parse(raw.toString());if(f.type==='input'&&evidence.inputs.length<18000)evidence.inputs.push({wallMs:Date.now()-since,seq:f.seq,input:f.input});});
 ...
 socket.send=function(raw,...args){
  ...
  if(f.type==='events') evidence.events.push(...f.items.map(item=>({wallMs:Date.now()-since,...item})));
...
const deadline=setTimeout(close,200000);
