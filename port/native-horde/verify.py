...
commands = [
    ['node', '--test', 'port/native-horde/test.mjs', 'port/native-horde/input-buffer.test.mjs', 'port/native-horde/repair-regression.test.mjs'],
...
with tempfile.TemporaryDirectory(prefix='horde-check-', dir='/tmp/opencode') as temporary:
    env = dict(os.environ)
    ...
    for index, command in enumerate(commands):
        ...
        output_dir = ROOT / 'port/reports/horde-repair'
        output_dir.mkdir(parents=True, exist_ok=True)
        (output_dir / f'check-{index}.log').write_text(output)
        print(output)
