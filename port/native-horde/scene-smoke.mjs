...
const env={...process.env};for(const k of ['XDG_DATA_HOME','XDG_CONFIG_HOME','XDG_CACHE_HOME']){env[k]=temp+'/'+k;mkdirSync(env[k]);}
let wave=false,properDefault=false,output='',code=1,child,timer;
const authority=createAuthority({observe(r){if(r.frame.type==='snapshot'){const s=r.frame.state;wave ||= s.singleplayer.wave===1&&s.singleplayer.enemiesAlive>0;properDefault ||= s.singleplayer.waveTarget===10;}}});
try{
...
writeFileSync('port/native-horde/product-smoke.log',output);
writeFileSync('port/native-horde/product-smoke.json',JSON.stringify(result,null,2));
