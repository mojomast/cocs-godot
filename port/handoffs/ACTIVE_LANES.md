...
| Player/operator model improvement | External staged delivery recovered on `review/recovered-player-models` against `d306205`; fresh numeric/live/preview checks pass; lead reviewed all40 matched images; integration HOLD for distance readability/render cost | `godot/player_models/`, `godot/tests/player_models/`, `port/native-player-models/`; see `port/native-player-models/REVIEW.md`; shared hooks remain unapplied; pulse-rifle reservation remains separate |
