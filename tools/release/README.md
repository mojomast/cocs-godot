...
- `godot/content/**` is gitignored, and the aggregate verifier imports two GLB probes
  ...
  for the locked revision (`npx --no-install playwright install chromium`);
  `PLAYWRIGHT_BROWSERS_PATH` is honoured, and the host's `$HOME/.cache/ms-playwright` is
  used when it exists. A generation failure stops at `probe-generation-failed` with that
  command in the hint. `--prepare=never` skips generation and lets the `glb-import` gate
  fail. The generated bytes are gitignored, so the tracked tree stays frozen.
