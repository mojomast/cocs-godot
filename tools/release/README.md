...
| 2 | `verification` | `python3 tools/godot-dev/verify.py` with `GODOT_BIN`, `PORT=0`, `TMPDIR=/tmp/opencode` and `XDG_*` redirected into the state dir | any non-zero exit, a `status != passed` report, or any failing gate |
