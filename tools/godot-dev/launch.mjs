...
const game=plan.endpoint ? null : createGameServer({historyPath:null,progressionPath:null});let child;
...
try{
 ...
 const args=[...plan.args,'--',`--endpoint=${endpoint}`,...plan.sessionOptions];
 child=spawn(binary,args,{env,stdio:'inherit'});
 process.exitCode=await new Promise((resolve,reject)=>{child.once('error',reject);child.once('exit',(code)=>resolve(code??1));});
}finally{stop();if(game)await game.close();process.removeListener('SIGINT',stop);process.removeListener('SIGTERM',stop);}
