...
try {
  ...
  // authority's own identity echo.
  {
    let result;
    try {
      result = await exec(process.env.ComSpec || 'cmd.exe', ['/d','/s','/c',`""${join(root,'Play.cmd')}" --experience=identity-zones --map=vermilion-fold --smoke"`], {cwd:sandbox,env,windowsVerbatimArguments:true,timeout:60000,maxBuffer:4*1024*1024});
    } catch (error) {
      await writeFile(join(output,'identity-zones-vermilion-fold.log'),(error.stdout || '')+(error.stderr || '')+'\n'+error.message);
      throw error;
    }
    const text = result.stdout+result.stderr;
    await writeFile(join(output,'identity-zones-vermilion-fold.log'),text);
    assert.doesNotMatch(text, /SCRIPT ERROR|ERROR:|Assertion failed/);
